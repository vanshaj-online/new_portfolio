            ...
            <div id='about' ref={textContainer} className='w-full flex flex-col md:flex-row items-center justify-center md:justify-evenly relative gap-12 md:gap-0'>
                <div className='md:h-screen w-max flex flex-col items-center justify-center'>
                    {text.map((elem, index) => (
                        <div key={index} className='overflow-hidden'>
                            <p ref={el => textRef.current[index] = el} className='voyage links leading-relaxed' style={{ fontSize: 'clamp(20px, calc(7vw + 0.5rem), 60px)' }}>
                                {elem}
                            </p>
                        </div>
                    ))}
                ...
                <div className='md:h-screen md:w-2/5 w-full flex items-center justify-center'>
                    <div className='h-auto w-max flex items-start justify-center overflow-hidden rounded-xl md:rounded-none'>
                        <img loading='lazy' src={myPhoto} ref={imgRef} className='md:h-[35vw] h-auto w-full md:w-auto opacity-0 object-cover object-bottom' alt="Vanshaj" />
                    </div>
                </div>
            </div>
        </div>
